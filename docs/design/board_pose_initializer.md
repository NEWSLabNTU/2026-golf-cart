...
src/localization/golfcart_board_initializer/
  ...
  test/
    sim/vlp32_sim.py  # synthetic scan generator
    sim/scenes.py     # room, board, and distractor definitions
    test_detector.py  # pytest, no ROS required
...
def detect_board(points: np.ndarray,      # (N, 3) in sensor frame
                 ...
                 params: DetectorParams) -> BoardDetection | None
```
...
the `velodyne` frame with no deskew. This roughly triples the returns on the
board and averages down range noise. Abort and restart accumulation if
...
keep &= (r >= 1.5) & (r <= 15.0)              # near: blooming; far: too few rings
...
Euclidean clustering, tolerance 0.20 m, minimum 20 points.

The tolerance is set by *across-ring* spacing, not within-ring spacing. At 10 m
the dense elevation band of the VLP-32C gives roughly 6 cm between rings, but the
sparse band is several times that. A tolerance tuned to within-ring spacing
splits the board into disconnected horizontal stripes.

Minimum point count should scale with range — expected hits are approximately
(board angular height / ring spacing) × (board angular width / azimuth step).
Below about 15 m this stays comfortably above 20.
...
| Density | point count within ±50% of the count predicted for the measured range | A distant large panel masquerading as a near small one |
...
`T_map←board` is the identity when the map is anchored to the board per the
mapping design §4, which is the payoff of anchoring. It stays a parameter
regardless, so an un-anchored map remains usable.
...
```
WAIT_TF → WAIT_MAP → ACCUMULATE → DETECT → CALL_SERVICE → DONE
                          ↑           │
...
| Axis | Values | Expected result |
|------|--------|-----------------|
| Range | 2, 5, 10, 15 m | Detect; error grows with range |
| Yaw to board | 0°, ±30°, ±60° | Detect; ≥70° may legitimately fail |
| Occlusion | 0%, 30% left, 50% bottom | Detect with widened covariance, or clean reject |
| Distractors only | no board present | **No detection** — false-positive test |
| Two boards | — | **Ambiguity abort** |
| Dirty board | intensity capped at 130 | Detect; threshold 110 has margin |
| Board tilted 10° | — | Detect; the verticality gate must not reject it |
| Blooming enabled | halo + 3–5 cm range inflation | Detect; bias below tolerance |

**Metrics**: position error, yaw error, detection rate, false-positive rate.
...
- Node initializes localization end to end in `logging_simulation` replay of an
  indoor bag, with no GNSS and no manual RViz input.
