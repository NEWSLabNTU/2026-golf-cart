...
"""Audit the two inputs the EKF prior is built from: IMU and VelocityReport.
    ...
    python3 scripts/localization/check_imu_velocity.py --seconds 200
...
def main() -> int:
    ...
    class Chk(Node):
        def __init__(self):
            ...
            self.create_subscription(Imu, "/sensing/camera/zed/imu/data", self.on_raw, be)
    ...
    for name, arr in (("imu_data (corrected)", imu), ("zed imu (raw)", raw),
    ...
    print(f"\n=== coordinates ===")
    print(f"  imu_data frame: {imu[0][1]}    zed raw frame: {raw[0][1] if raw else '-'}")
