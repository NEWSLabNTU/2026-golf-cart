...
#     `uname -r` -- here 5.15.148-tegra -- finds only linux-tools-5.15.0-177,
#     and refuses with "WARNING: perf not found for kernel 5.15.148". The ABI is
#     identical across a 5.15 point release, so we call the real binary under
#     /usr/lib/linux-tools/*/perf directly.
...
set -euo pipefail
SECS="${1:-20}"
OUT="${GOLFCART_PERF_DIR:-/tmp}/golfcart-perf.data"
