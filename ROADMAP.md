...
1. ~~**Review AutoSDV planning/MRM patches**~~ **Done 2026-08-21: there were no patches.** The audit found that `golfcart_launch/config/` held 181 files of which only 35 were reachable from any launch file, and that 145 of the unreachable ones were byte-identical to `b378c60` (2025-12-28, "Copy autoware_launch config files to autosdv_launch"). Nobody had ever edited them, so every apparent delta against current upstream was version drift, not intent. Control, planning, map and simulator params all came from `autoware_launch` at runtime. The 145 were deleted; localization (the one subtree that is wired, and the only one anyone tuned) is untouched. See the sweep commit for the full reasoning.
