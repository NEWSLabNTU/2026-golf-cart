...
All three of the sample bag's LiDARs are preprocessed: top (VLS128), left and
right (VLP16). They all arrive as `velodyne_packets`, and the driver container
(`nebula_driver_container.launch.py`, run with `run_preprocessing:=false`)
decodes them to `pointcloud_raw_ex` in the `PointXYZIRCAEDT` layout, which is
what both chains need.

Mirrors golfcart_sensor_kit_launch's copy so the two kits can be compared
directly. Kept as a copy rather than shared, because a sensor kit must build
without depending on another kit.
"""
...
PREPROCESSED_LIDARS = [
    ("top", "pointcloud_raw_ex"),
    ("left", "pointcloud_raw_ex"),
    ("right", "pointcloud_raw_ex"),
]

# Nothing bypasses preprocessing in this kit; every LiDAR in the sample bag
# carries the extended point layout. The golf cart kit has a PASSTHROUGH_LIDARS
# list because its Seyond driver does not.
PASSTHROUGH_LIDARS = []
...
def generate_launch_description():
    ...
    kit = get_package_share_directory("sample_bag_sensor_kit_launch")
