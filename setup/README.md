...
Nothing here imports anything outside the standard library. The menu was
Textual, which cost 5.8 MB of widgets inside a 15 MB venv and about 54 s on a
cold first run while `uv` fetched a Python and resolved dependencies, to draw a
list of 25 checkboxes. `curses` is stdlib, imports in 3 ms, and took the venv,
`requirements.txt` and the `uv` bootstrap with it when it replaced Textual.
...
| `vehicle` | the cart: everything in `dev`, plus sensor udev, CAN, PTP, camera kernel modules |
...
`vehicle` is `dev` plus one group. The steps that touch devices, device naming
or the bus live under **System config**, and that group is the entire difference
between the two — so a workstation never writes a udev rule, and the cart never
misses one.

`all` and `none` are computed rather than declared, so a new step joins them
without anyone remembering to.

The suggested profile comes from detection — whether CAN interfaces, a u-blox
receiver or a TIER IV camera are attached, and `config/host` — and is a starting
point, never a restriction. There is no per-board profile: a Jetson on a desk is
a development machine and a Jetson in the cart is the vehicle, and what
separates them is the hardware, not the SoC. A step whose hardware is absent is shown as such and
